...
const userSerivces = require("../Service/userService");
...
const register = async (req, res) => {
  ...
  if (userSerivces.validateEmailAndPassword(email, password)) {
    if (await userSerivces.registerUser(email, name, password)) {
      res.status(201).json({ message: "User registered successfully!" });
    } else {
      res.status(500).json({
        error: "Internal server error occurred while creating the account.",
      });
    }
  } else {
    res.status(409).json({
      error: "email must have @ and password must longer than 8 characters",
    });
  }
};
